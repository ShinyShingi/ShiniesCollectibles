...
class AbeBooksService extends AbstractPriceCheckingService
{
    ...
    protected function buildSearchUrl(Item $item): string
    {
        ...
        return 'https://www.abebooks.com/servlet/SearchResults?' . http_build_query($params);
    }
    ...
    protected function parseResults(string $html, Item $item): array
    {
        ...
        try {
            ...
            // AbeBooks result selectors (these may need updating based on their current HTML structure)
            $crawler->filter('.cf.result-item')->each(function (Crawler $node) use (&$results, $item) {
                try {
                    ...
                    $url = $linkNode->count() > 0 ? 'https://www.abebooks.com' . $linkNode->attr('href') : '';
                    ...
                    if ($price && $title) {
                        $results[] = [
                            ...
                            'isbn' => $item->isbn,
                            'price' => $price,
                            'shipping_cost' => 0, // Will be updated if shipping info is found
        ...
        return array_slice($results, 0, 10); // Limit to top 10 results
    }
}